...
Файл: [`out/YouTubeVot.apk`](out/YouTubeVot.apk) (~0,4 МБ, подписан debug-ключом, версия 1.3.0)
...
- **Material-иконки** вместо текстовых кнопок: назад, вперёд, обновить, выход
  (VectorDrawable 24dp, тач-таргеты 48dp, contentDescription для доступности).
- **Навигация по истории** WebView: кнопки «назад/вперёд» активны только когда есть куда ходить.
- **Тонкий прогресс-бар** загрузки над страницей (акцентный цвет, исчезает при 100%).
- **Полноэкранное видео:** при входе в полноэкранный режим приложение переключается
  в ландшафтную ориентацию и прячет верхнюю панель, при выходе — возвращается в портретную.
- **Тёмные системные панели:** статус-бар скрыт (вид медиа-приложения), нижняя
  навигационная панель в цвет фона YouTube.
- **Без белых вспышек:** фон WebView и оверскролл в `#0F0F0F`, цветное «свечение» краёв отключено.
- **На странице скрываются** банеры «Откройте в приложении YouTube» и cookie/consent-попапы
  (CSS + MutationObserver-чистильщик в bootstrap.js — узлы удаляются, а не только скрываются).

### Плавность и «премиальность» (1.3)

- **Ripple-отклик** на всех кнопках верхней панели (`selectableItemBackgroundBorderless`) +
  лёгкая **вибрация** (haptic) при нажатии.
- **Плавное появление страницы:** при навигации WebView прозрачен, затем страница
  мягко проявляется за ~220 мс — никаких резких смен кадра.
- **Адаптивная иконка** (API 26+): тёмный фон + белый «play», корректная маска
  производителя; на старых версиях — прежние PNG.
- **Нативный сплэш Android 12+**: тёмный экран запуска с иконкой (без белого старта),
  на старых версиях — тёмный фон окна.
- **Скроллбары WebView скрыты** — чище вид, как у нативного приложения.
...
### Автосборка и релизы на GitHub Actions

Сборку и публикацию можно полностью отдать GitHub. Пуш тега `vX.Y.Z` запускает
Windows-раннер: он скачивает JDK 17 + Android SDK (build-tools 34, platform 35),
выполняет тот же `build.ps1` и сам создаёт GitHub Release с APK:

```powershell
git tag v1.3.0
git push origin v1.3.0   # -> релиз появится автоматически
```

Вручную проверить сборку без релиза: **Actions → Build APK → Run workflow**.
